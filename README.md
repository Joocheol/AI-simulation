...
금융회사 실무자를 대상으로 한 **AnyLogic 시뮬레이션 입문 과정(총 5시간, 2.5시간 × 2회)** 의 커리큘럼 설계 문서 모음입니다.
...
| [05 · 운영과 라이선스](docs/05-operations-and-license.md) | 사전 준비, **망분리 대응**, **PLE 라이선스 정책**, 리스크 레지스터 | 교육담당자·구매/법무 |
...
이 세 가지는 커리큘럼 설계보다 **상위의 실행 가능성 문제**입니다. 해결되지 않으면 어떤 시간표도 의미가 없습니다.

1. **PLE 기술 제약 실측** — 모델시간 5시간 제한이 실제로 어디에 걸리는지. 특히 **몬테카를로 복제가 합산 계산되는지**(합산이면 M2가 통째로 막힙니다). → [07 · 검증 프로토콜](docs/07-ple-verification.md)
2. **라이선스** — PLE는 "classroom instruction and personal learning" 한정이며 **상업적 사용을 금지**합니다. 영리 금융회사 대상 사내교육의 해당 여부를 **서면으로 확인**해야 합니다. → [05 · 라이선스](docs/05-operations-and-license.md#3-소프트웨어-라이선스-정책)
3. **망분리 환경** — 국내 은행 내부망 PC는 통상 외부 SW 설치·인터넷 접속이 금지되고, PLE는 온라인 라이선스 활성화가 필요합니다. 외부망 교육장 또는 전용 장비 확보를 **계약 조건에 명시**해야 합니다. → [05 · 교육 환경](docs/05-operations-and-license.md#2-교육-환경-및-기술-요건)
