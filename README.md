...
학교에서 진행하기 때문에 **라이선스 분쟁은 발생하지 않고, 수강생 개인의 설치 부담도 사라집니다.** PLE 라이선스는 *"teaching or self-education purposes only"* 를 명시적으로 허용하므로 대학 주관 교육은 정확히 그 허용 범위이고, 전산실 PC에 일괄 설치하면 사내 PC의 설치 제약도 우회됩니다.

> ⚠ **다만 네트워크 문제가 전부 사라지는 것은 아닙니다.** PLE는 **온라인 라이선스 활성화**가 필요하고, 학교 방화벽이 이 통신을 막을 수 있습니다(R4). 설치 부담이 없어진 것과 활성화 통신이 통과하는 것은 **별개**이므로, D-7 설치 시점에 반드시 확인하고 오프라인 활성화 경로를 확보해 두십시오.
...
3. **라이선스 활성화 통신** — 학교 방화벽이 PLE 활성화를 차단하면 D-7 일괄 설치가 통째로 실패합니다. 오프라인 활성화 가능 여부를 [07 · A5](docs/07-ple-verification.md#3-함께-확인할-항목)에서 확인하십시오.
